...
const SignupPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!/\S+@\S+\.\S+/.test(input?.email)) return toast.error("Invalid email format");
    if (!input.password) return toast.error("Password is required");
    if (input?.password?.length < 6) return toast.error("Password must be at least 6 characters");

    return true;
  };

  const onChangeEventHanlder = (e) => {
